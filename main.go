...
func main() {
	...
	wwwdir := getEnv("HTTP_DIR", "/var/www")
	serverPort := getEnv("APIO_PORT", "8080")
	...
	// router.PathPrefix("/download/files/collector").Handler(http.StripPrefix("/download/files/collector", http.FileServer(http.Dir("/media/test"))))
